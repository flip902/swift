...
class ViewController: NSViewController {
    ...
    let defaultText = "Sarah Fong is one bad Bitch!"
    ...
    fileprivate func setWordsToSpeak() {
        ...
        if let speachText = contents, speachText.isEmpty {
            wordsVoiceHasToSpeak.maxValue = Double(speachText.count)
        } else {
            wordsVoiceHasToSpeak.maxValue = Double(defaultText.count)
        }
    }
